...
page.on("response", async (response) => {
  ...
  try {
    if (
      ...
      if (/octet-stream|application\/x-protobuf|pbf/i.test(contentType)) {
        captured.push(row);
        return;
      }
...
for (const label of INTERACTION_TARGETS) {
  const clicked = await safeClick(page, label);
  interactionResults.push({ label, clicked: clicked ? "true" : "false" });
}
...
const keywordUrls = urls.filter(urlKeywordMatch);
const arcgisObjects = keywordUrls.map(serviceRoot).filter(Boolean);
const serviceRoots = unique(arcgisObjects.map((x) => x.root));
const observedLayerIdsByRoot = new Map();
for (const obj of arcgisObjects) {
  if (!obj.layerId) continue;
  if (!observedLayerIdsByRoot.has(obj.root)) {
    observedLayerIdsByRoot.set(obj.root, new Set());
  }
  observedLayerIdsByRoot.get(obj.root).add(String(obj.layerId));
}

const serviceRows = [];
const fieldHitRows = [];
const layerRows = [];
const allLayerFieldRows = [];
...
for (const root of serviceRoots) {
  ...
  const layerIds = new Set(layers.map((l) => String(l.id)).filter(Boolean));
  const observedLayerIds = observedLayerIdsByRoot.get(root);
  if (observedLayerIds) {
    for (const id of observedLayerIds) layerIds.add(String(id));
  }
  const layerNameById = new Map(layers.map((l) => [String(l.id), l.name || ""]));

  for (const layerId of layerIds) {
    if (layerId === undefined || layerId === null || layerId === "") continue;
    ...
    const resolvedLayerName = layerNameById.get(String(layerId)) || layerMeta.json?.name || "";
    ...
    for (const f of fields) {
      allLayerFieldRows.push({
        ...
        field_nullable: f.nullable ?? "",
        field_editable: f.editable ?? ""
      });
...
const audit = {
  ...
  keyword_matched_url_count: keywordUrls.length,
  ...
  arcgis_layer_field_count: allLayerFieldRows.length,
  ...
  interaction_click_results: interactionResults,
  interaction_click_success_count: interactionResults.filter((x) => x.clicked === "true").length,
  ...
  outputs: {
    ...
    arcgis_layer_fields_csv: "huntplanner_arcgis_layer_fields.csv",
...
writeCsv(path.join(OUT_DIR, "huntplanner_arcgis_layer_fields.csv"), allLayerFieldRows, [
  "service_root",
  "layer_id",
  "layer_name",
  "field_name",
...
fs.writeFileSync(
  path.join(OUT_DIR, "huntplanner_network_urls.txt"),
  [
    "# Keyword-matched URLs",
    ...keywordUrls,
    "",
    "# All captured URLs",
    ...urls
