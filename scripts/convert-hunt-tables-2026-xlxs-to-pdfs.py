...
def ptxt(s: str, style: ParagraphStyle) -> Paragraph:
    txt = (s or "").replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
...
def render_pdf(xlsx: Path, pdf: Path) -> Tuple[int, int]:
    ...
    st_head = ParagraphStyle("h", parent=styles["Normal"], fontName="Helvetica-Bold", fontSize=8.1, leading=9.2, textColor=colors.white)
    st_cell = ParagraphStyle("c", parent=styles["Normal"], fontName="Helvetica", fontSize=7.5, leading=8.8)

    data = [[ptxt(h, st_head) for h in headers]]
    for r in rows:
        data.append([ptxt(v, st_cell) for v in r])
