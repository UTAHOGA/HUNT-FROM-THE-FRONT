...
def test_retired_codes_removed_from_current_database_and_preserved_in_ledger() -> None:
    ...
    assert len(database_rows) == len(database_codes) == 1408
    ...
    assert summary["retired_codes"] == sorted(RETIRED_CODES)
    assert summary["retired_row_count"] == 3
    assert summary["database_row_count_after"] == 1408
