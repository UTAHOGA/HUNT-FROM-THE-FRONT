...
def test_hunt_code_family_gap_scan_runs_and_writes_outputs() -> None:
    ...
    assert summary["predictive_v2_code_count"] == 1119
    ...
    assert summary["total_missing_predictive_v2_current_database_codes"] == 352
    assert summary["total_required_surface_missing_current_database_codes"] == 0
    assert set(summary["resolved_families"]) == {"BR", "GO", "MA", "MB", "PB", "TK"}
...
def test_hunt_code_family_gap_scan_ranks_largest_predictive_gaps() -> None:
    ...
    assert [item["code_prefix"] for item in ranked[:3]] == ["EL", "LO", "EA"]
    assert ranked[0]["missing_predictive_v2_count"] == 126
    assert ranked[1]["missing_predictive_v2_count"] == 113
    assert ranked[2]["missing_predictive_v2_count"] == 51
    assert "EL3000" in ranked[0]["missing_predictive_v2_codes"]
    assert "LO0008" in ranked[1]["missing_predictive_v2_codes"]
    assert "EA1007" in ranked[2]["missing_predictive_v2_codes"]


def test_hunt_code_family_gap_scan_marks_bear_and_turkey_resolved() -> None:
    ...
    assert rows["EA"]["status"] == "PREDICTIVE_GAP"
