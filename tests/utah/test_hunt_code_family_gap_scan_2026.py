...
def test_hunt_code_family_gap_scan_runs_and_writes_outputs() -> None:
    ...
    assert summary["predictive_v2_code_count"] == 1457
    assert summary["family_count"] == 21
    assert summary["resolved_family_count"] == 16
    assert summary["predictive_gap_family_count"] == 5
...
def test_hunt_code_family_gap_scan_ranks_largest_predictive_gaps() -> None:
    ...
    assert [item["code_prefix"] for item in ranked[:3]] == ["LD", "RS", "BI"]
    assert ranked[0]["missing_predictive_v2_count"] == 6
    assert ranked[1]["missing_predictive_v2_count"] == 4
    assert ranked[2]["missing_predictive_v2_count"] == 2
    assert "LD1001" in ranked[0]["missing_predictive_v2_codes"]
    assert "RS1000" in ranked[1]["missing_predictive_v2_codes"]
    assert "BI6527" in ranked[2]["missing_predictive_v2_codes"]
...
def test_hunt_code_family_gap_scan_marks_resolved_reference_families() -> None:
    ...
    for prefix in ("DA", "DB", "DS", "EA", "EB", "EL", "LO", "LP", "PD", "RE"):
    ...
    assert rows["LD"]["status"] == "PREDICTIVE_GAP"
