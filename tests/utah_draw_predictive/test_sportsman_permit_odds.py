...
def test_sportsman_rows_stay_pending_without_usable_odds_source() -> None:
    ...
    assert all(row.get("algorithm_status") == "MODELED_SPORTSMAN_DRAW" for row in rows)
    assert all((row.get("residency") or "").strip() == "Resident" for row in rows)
    assert all((row.get("p_sportsman_draw") or "").strip() != "" for row in rows)
    assert all((row.get("p_draw") or "").strip() == (row.get("p_sportsman_draw") or "").strip() for row in rows)
